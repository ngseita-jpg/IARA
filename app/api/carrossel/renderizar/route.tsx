...
function hexToRgb(hex: string): [number, number, number] {
  ...
  return [((n >> 16) & 255) || 0, ((n >> 8) & 255) || 0, (n & 255) || 0]
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const total = typeof total_slides === 'number' ? total_slides : 0
    ...
    const dots = total > 0 ? Array.from({ length: Math.min(total, 10) }, (_, i) => i) : []

    return new ImageResponse(
      (
        <div style={{ display: 'flex', width: '100%', height: '100%', position: 'relative', overflow: 'hidden', fontFamily: 'Inter', backgroundColor: pri }}>

          {/* Gradiente de fundo (sem imagem) */}
          {!hasBg && (
            <div style={{
              position: 'absolute', top: 0, left: 0, width: '100%', height: '100%',
              backgroundImage: `linear-gradient(135deg, ${pri} 0%, ${sec} 100%)`,
              display: 'flex',
            }} />
          )}

          {/* Círculos decorativos (sem imagem) */}
          {!hasBg && (
            <div style={{
              position: 'absolute', width: 700, height: 700, borderRadius: '50%',
              backgroundColor: `rgba(${pr},${pg},${pb},0.3)`,
              top: -250, right: -250, display: 'flex',
            }} />
          )}
          ...
          {hasBg && (
            ...
            <img src={imagem_base64!} alt="" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
          )}
          ...
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 72, display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingLeft: 48, paddingRight: 48 }}>
            ...
            {isCapa ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                {dots.map(i => (
                  <div key={i} style={{ width: i === slide.ordem - 1 ? 26 : 8, height: 8, borderRadius: 999, backgroundColor: i === slide.ordem - 1 ? '#ffffff' : 'rgba(255,255,255,0.28)', display: 'flex' }} />
                ))}
              </div>
            ...
            {/* Número */}
            {!isCapa && (
              <span style={{ fontSize: 15, fontWeight: 700, color: 'rgba(255,255,255,0.50)', letterSpacing: '0.04em' }}>
          ...
          {/* Conteúdo */}
          <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: p.jc, alignItems: p.ai, paddingTop: p.py + 36, paddingBottom: p.py, paddingLeft: p.px, paddingRight: p.px, gap: 0 }}>
            ...
            {/* Pill decorativo (conteúdo) */}
            {!isCapa && !isEncerramento && (
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: 20, alignSelf: p.ai }}>
                <div style={{ width: 32, height: 4, borderRadius: 999, backgroundColor: sec, marginRight: 10, display: 'flex' }} />
            ...
            {slide.corpo && (
              <div style={{
                ...
                display: 'flex',
                flexWrap: 'wrap',
                ...(slide.cor_fundo_texto && !isCapa ? { backgroundColor: slide.cor_fundo_texto.replace(/[\d.]+\)/, '0.30)'), paddingTop: 10, paddingBottom: 10, paddingLeft: 18, paddingRight: 18, borderRadius: 10 } : {}),
              }}>
                {slide.corpo}
              </div>
            )}
            ...
            {/* CTA */}
            {slide.cta && (
              <div style={{ marginTop: 28, display: 'flex', alignItems: 'center', alignSelf: p.ai }}>
                <div style={{ fontSize: Math.round(fs * 0.46), fontWeight: 700, color: contrastColor(sec), backgroundColor: sec, paddingTop: 16, paddingBottom: 16, paddingLeft: 36, paddingRight: 36, borderRadius: 999, letterSpacing: '0.03em', display: 'flex' }}>
                  {slide.cta}
                </div>
              </div>
            )}
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 5, backgroundImage: `linear-gradient(90deg, ${pri} 0%, ${sec} 50%, ${pri} 100%)`, display: 'flex' }} />

          {/* Hint "arraste" na capa */}
          {isCapa && (
            <div style={{ position: 'absolute', bottom: 40, right: 48, display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.38)', letterSpacing: '0.08em' }}>ARRASTE</span>
              <div style={{ width: 7, height: 7, borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.65)', display: 'flex' }} />
              <div style={{ width: 7, height: 7, borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.25)', display: 'flex' }} />
              <div style={{ width: 7, height: 7, borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.25)', display: 'flex' }} />
            </div>
          )}
        </div>
    ...
    console.error('Renderizar error:', err)
