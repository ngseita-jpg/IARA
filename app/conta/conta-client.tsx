...
export function ContaClient() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      // Tenta refrescar sessao Supabase antes (cobre cookie quase-expirado)
      try {
        const supabase = createClient()
        await supabase.auth.getSession()
      } catch { /* noop, segue tentando */ }

      const r = await fetch('/api/perfil/conta', {
        cache: 'no-store',
        credentials: 'include',
        headers: { 'x-iara-source': 'conta-page' },
      })
      if (r.status === 401) {
        // Re-autentica via refresh — se ainda 401, manda pro login
        const supabase = createClient()
        const { data, error: refreshErr } = await supabase.auth.refreshSession()
        if (refreshErr || !data.session) {
          router.push('/login?mensagem=sessao-expirada')
          return
        }
        // Tenta UMA VEZ depois do refresh
        const r2 = await fetch('/api/perfil/conta', { cache: 'no-store', credentials: 'include' })
        if (!r2.ok) {
          router.push('/login?mensagem=sessao-expirada')
          return
        }
      ...
      if (!r.ok) {
        ...
        // Reporta erro pro audit log pra investigacao
        void fetch('/api/audit/evento', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            evento: 'conta_carregar_erro',
            meta: { status: r.status, error: body.error?.slice(0, 200) },
          }),
        }).catch(() => null)
      ...
      const msg = e instanceof Error ? e.message : 'Erro ao carregar conta'
      setErroCarregar(msg)
      // Audit do erro client (pra ver se rede caiu, CORS, etc.)
      void fetch('/api/audit/evento', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          evento: 'conta_carregar_erro_client',
